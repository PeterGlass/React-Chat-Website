...
function Chat(props) {
    ...
    const getUserCount = () => {
        Axios.get('https://onlypeters.com/api/getUserCount').then((response) => {
            setUserCount(response.data[0].user_count);
        });
    };

    const startSession = () => {
        Axios.get('https://onlypeters.com/api/startSession',{
            params: {
                alias: tempAlias
            }
        }).then((response) => {
    ...
    const endSession = () => {
        Axios.delete('https://onlypeters.com/api/endSession',{
            data: {
                sessionId: sessionId
            }
        });
    ...
    const getMessageLog = () => {
        Axios.get("https://onlypeters.com/api/getMessage").then((response) => {
    ...
    const sendMessage = (e) => {
        e.preventDefault();
        Axios.post('https://onlypeters.com/api/insertMessage', {
            alias: alias, 
            message: message
        });
        setMessage('');
    ...
    const createUser = (e) => {
        e.preventDefault();
        Axios.get("https://onlypeters.com/api/isAliasUnique", {
            params: {
                alias: tempAlias
            }
        }).then((response) => {
            if(response.data){
                if(tempAlias <= 20){ 
                    ...
                    setIsLogin(true);
                    startSession();
                    Axios.post("https://onlypeters.com/api/insertUser", {
                        alias: tempAlias,
                        password: password
                    });
                } else {
    ...
    const userLogin = (e) => {
        e.preventDefault();
        Axios.get("https://onlypeters.com/api/isUserAuth", {
            params: {
                alias: tempAlias,
                password: password
            }
